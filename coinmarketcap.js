...
function toggle_native() {
    $("#currency-switch-button").html("Native" + " <span class=\"caret\"></span>");
...
$(document).ready(function() {
    if (window.location.hash) {
        ...
        if (hash == "BTC" || hash == "USD" || hash == "EUR" || hash == "CNY" || hash == "GBP" || hash == "CAD" || hash == "RUB" || hash == "HKD" || hash == "JPY" || hash == "AUD" || hash == "BRL" || hash == "INR" || hash == "KRW" || hash == "MXN" || hash == "IDR" || hash == "CHF") {
...
CurrencyDetailGraph.prototype.fetchAndLoad = function(callback, start, end) {
    ...
    $.ajax({
        url: apiDomain + "/currencies/" + slug + "/" + timeParams,
        type: "GET",
        dataType: "json",
        error: function() {
            that.hideLoading();
